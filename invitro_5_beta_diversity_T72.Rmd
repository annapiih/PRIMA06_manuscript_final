---
title: "In vitro Beta Diversity T72"
author: "apih"
date: "`r Sys.Date()`"
output:
...
# Create subset
phy.clean <- subset_samples(phy, Time_point == "T72")
...
# Save object
save(phy.rare, phy.clean, file = "R_objects/Phyloseq_betadiv_T72.Rdata")
...
# load
params <- readRDS("R_objects/params_betadiv_invitro.RDS")
load("R_objects/Phyloseq_betadiv_T72.Rdata")
...
save(ait.dist, ait.nmds, ait.pcoa, phy.clean, file = "R_objects/bdiv_ait_T72.RData")
...
# Save distance objects
save(jac.dist, jac.nmds, jac.pcoa, phy.rare, file = "R_objects/bdiv_jac_T72.RData")
...
if (METRIC == "jac") {
  load("R_objects/bdiv_jac_T72.RData")
  dist.used <- jac.dist
  nmds.used <- jac.nmds
  pcoa.used <- jac.pcoa
  phy.used <- phy.rare
  ...
  load("R_objects/bdiv_ait_T72.RData")
  dist.used <- ait.dist
  nmds.used <- ait.nmds
  pcoa.used <- ait.pcoa
  phy.used <- phy.clean
...
plot.pcoa.box <- annotate_figure(
  plot.pcoa.box,
  top = text_grob("Aitchison distance 72h (ASVs)", face = "bold", size = 12),
)

plot.pcoa.box


filename <- paste0("plots/In_vitro/aitchison_t72_asv.png")
plot.pcoa.box
...
plot.pcoa.box <- annotate_figure(
  plot.pcoa.box,
  top = text_grob("Jaccard 72h (ASVs)", face = "bold", size = 12),
)

plot.pcoa.box

filename <- paste0("plots/In_vitro/jaccard_t72_asv.png")
plot.pcoa.box
suppressMessages(ggsave(filename = filename, width = 1800, height = 1400, units = "px", dpi = "print", device = "png", ))
