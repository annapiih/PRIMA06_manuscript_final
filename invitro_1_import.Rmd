...
knit: (function(inputFile, encoding) { 
    rmarkdown::render(
        ...
        output_file = paste0("PRIMA08_", Sys.Date(), "_ImportQC.html")) 
    })
params:
    input: "input/Lop_invitro.RData"
    meta: "input/lop_invitro_meta.csv"
    neg: "Water|Neg|NC|ctrl|Qneg|Vand"
    batch: "Seq_run"
...
# Save params
saveRDS(params, file = "R_objects/import_params_invitro.RDS")
```
...
sample_data(phy)$SampleID <- with(sample_data(phy), paste(Sample, sep = "_"))
...
meta <- read_csv("input/lop_invitro_meta.csv")

# Create an identical ID variable to use for merging
meta$SampleID <- with(meta, paste(Sample, sep = "_"))
...
} else cat("All samples from sequencing data matched in metadata")

if (nrow(anti_join(meta, tmp, by = "SampleID")) != 0) {
  anti_join(meta, tmp, by = "SampleID")
} else cat("All samples from metadata matched in sequencing data")
...
save(phy.org, phy, file="R_objects/input_invitro.Rdata")
...
*Merged phyloseq object saved in: "**R_objects/input_invitro.Rdata**"*
...
################################################################################
# load data 
load("R_objects/input_invitro.Rdata")
...
save(phy.org, phy, file="R_objects/cleaned_invitro.Rdata")
...
params <- readRDS("R_objects/import_params_invitro.RDS")

# load data 
load("R_objects/cleaned_invitro.Rdata")
...
suppressMessages(ggsave("plots/in_vitro/sequencing_depth.pdf"))
...
sample_data(phy)$DNA_conc <- ifelse(is.na(sample_data(phy)$DNA_conc),
                                    0,
...
save(contam.df, contam.long, file = "R_objects/Decontam_tables_invitro.RData")
...
suppressMessages(ggsave(decontam.plot,file = "plots/in_vitro/contaminant_fraction_multiple.png",device = "png"))


```

*complete decontam classifications is saved in: "**R_objects/Decontam_tables_invitro.RData**"*
...
load("R_objects/Decontam_tables_invitro.RData")
...
# Evaluate what you can agree to loose and then use that column. I will use the Freq.none
...
suppressMessages(ggsave("plots/In_vitro/sequencing_depth_decontam.pdf"))
...
suppressMessages(ggsave("plots/In_vitro/sequencing_depth_decontam_harsh.pdf"))
...
save(phy, phy.harsh, file="R_objects/Decontam_invitro.Rdata")
...
tmp.contam <- data.frame(ASV = contam.df$ASV, contam_phy = contam.df$Freq.none, contam_harsh = contam.df$Freq.none)
tmp.out <- full_join(tmp.samples, tmp.contam, by = "ASV")

write_csv(tmp.out,file = "output/Decontam_Overview_invitro.csv")
...
*Cleaned phyloseq object saved in: "**R_objects/Decontam_invitro.Rdata**"*
...
# load data
load("R_objects/Decontam_invitro.Rdata")
...
suppressMessages(ggsave("plots/In_vitro/test_mock_comparison.png",mock.plot,device = "png"))
...
# save Rdat
save(Rdat.m, file = "R_objects/Rare_dat_invitro.RData")
...
# save Rdat
save(Rdat.m, file = "R_objects/Rare_dat_harsh_invitro.RData")
...
params <- readRDS(file = "R_objects/import_params_invitro.RDS")

# Load data
load("R_objects/Rare_dat_invitro.RData")

## ALL SEQ RUNS ## 
# plot per sample 
plot.ind <- ggplot(Rdat.m, aes_string(x = "depth", y = "Alpha_diversity")) + 
  geom_smooth(aes(group = Sample), se = FALSE) + 
  facet_wrap("Index", scales = "free",nrow = 1) + 
  geom_vline(color = "red",xintercept = 15000) + 
  theme_pubclean() + scale_color_brewer(palette = "Paired")
suppressMessages(ggsave(filename = "plots/In_vitro/Rcurve_individual.png",plot = plot.ind, device = "png"))
...
plot.group <- ggplot(Rdat.m, aes(x = depth, y = Alpha_diversity, color = Rdat.m$type)) + 
  geom_smooth(aes(group = Batch_type), method = "loess", formula = y ~ x, se = FALSE) + 
  facet_wrap("Index", scales = "free",nrow = 1) + 
  geom_vline(color = "red",xintercept = 20000) + 
  theme_pubclean() + scale_color_brewer(palette = "Paired")
suppressMessages(ggsave("plots/In_vitro/Rcurve_grouped.png", plot = plot.group, device = "png"))
...
# Load data
load("R_objects/Rare_dat_harsh_invitro.RData")
...
plot.ind <- ggplot(Rdat.m, aes_string(x = "depth", y = "Alpha_diversity")) + 
  ...
  geom_vline(color = "red",xintercept = 20000) + 
  theme_pubclean() + scale_color_brewer(palette = "Paired")
suppressMessages(ggsave(filename = "plots/In_vitro/Rcurve_individual_harsh.png",plot = plot.ind, device = "png"))
...
filename <- paste0("plots/In_vitro/Rcurve_grouped_harsh.png")
plot.group 
suppressMessages(ggsave(filename = filename, width = 2800, height = 1500, units = "px", dpi = "print", device = "png", ))
...
save(phy, file="R_objects/Phyloseq_invitro.Rdata")

# remove low read samples and mock from harshly decontaminated
phy <- prune_samples(sample_sums(phy.harsh) > 15000, phy.harsh)
phy <- subset_samples(phy, type == "Sample")
phy <- prune_taxa(taxa_sums(phy) > 0, phy)
phy_tree(phy) <- midpoint(phy_tree(phy))

# save harshely decontaminated samples
save(phy, file="R_objects/Phyloseq_harsh_invitro.Rdata")
...
# load data
load("R_objects/Phyloseq_invitro.Rdata")
...
# Save the phyloseq object
save(phy, INDECES, file="R_objects/Phyloseq_invitro.Rdata")
...
# Save the phyloseq object
save(phy, INDECES, file="R_objects/Phyloseq_harsh_invitro.Rdata")
