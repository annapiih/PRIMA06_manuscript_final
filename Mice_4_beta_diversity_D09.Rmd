...
# Create subset
phy.clean <- subset_samples(phy, Day == "D09")
...
# Save object
save(phy.rare, phy.clean, file = "R_objects/Phyloseq_betadiv_D09.Rdata")
...
# load
params <- readRDS("R_objects/params_betadiv.RDS")
load("R_objects/Phyloseq_betadiv_D09.Rdata")
...
save(ait.dist, ait.nmds, ait.pcoa, phy.clean, file = "R_objects/bdiv_ait_D09.RData")
...
# Save distance objects
save(jac.dist, jac.nmds, jac.pcoa, phy.rare, file = "R_objects/bdiv_jac_d09.RData")
...
if (METRIC == "jac"){
  load("R_objects/bdiv_jac_d09.RData")
  dist.used <- jac.dist
  nmds.used <- jac.nmds
  pcoa.used <- jac.pcoa
  phy.used <- phy.rare
  ...
  load("R_objects/bdiv_ait_d09.RData")
  dist.used <- ait.dist
  nmds.used <- ait.nmds
  pcoa.used <- ait.pcoa
  phy.used <- phy.clean
...
#Sort group variable
str(ord.tab$Group)
ord.tab$Group <- factor(ord.tab$Group, levels = c("Control", "Low", "Medium", "High"), ordered = TRUE)

my_cols <- c("firebrick","lightskyblue", "forestgreen","darkorchid")
...
plot.pcoa.box <- annotate_figure(
  plot.pcoa.box,
  top = text_grob("Aitchison distance (ASVs)", face = "bold", size = 12),
)

plot.pcoa.box


filename <- paste0("plots/Beta_diversity/aitchison_d09_asv.png")
plot.pcoa.box
...
# Pairwise test
permanova_pairwise(x = dist.used, grp = mdat$Group, padj = "fdr")
...
plot.pcoa.box <- annotate_figure(
  plot.pcoa.box,
  top = text_grob("Jaccard (ASVs)", face = "bold", size = 12),
)

plot.pcoa.box

filename <- paste0("plots/Beta_diversity/jaccard_d09_asv.png")
plot.pcoa.box
suppressMessages(ggsave(filename = filename, width = 1800, height = 1400, units = "px", dpi = "print", device = "png", ))
